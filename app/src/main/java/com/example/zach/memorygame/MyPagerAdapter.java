...
public class MyPagerAdapter extends PagerAdapter{
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        try {
            ...
            int[] backgrounds = new int[]{R.color.cartoon_theme_select_background,R.color.murica_theme_select_background};
            switch (adapterType)
            {
                case theme_main.ADAPTER_TYPE_TOP:
                    linMain.setBackgroundColor(Color.TRANSPARENT);
                    Glide.with(context)
                            .load(listItems[position])
                            .into(imageCover);

                    break;
                case theme_main.ADAPTER_TYPE_BOTTOM:
                    linMain.setBackgroundResource(backgrounds[position]);
                    break;
            }
